...
if (!AR.build.mobile) {
    ...
    AR.ADE.Screen = function() {
        ...
        var f = function() {
            ...
            r.onchange = function() {
                var H = AR.__fromJSONString__(r.value);
                document.getElementById("currentLatitude").value = H[0];
                document.getElementById("currentLongitude").value = H[1];
                document.getElementById("currentAltitude").value = H[2]
            };
            r.addOption = function(J, K, I) {
                var H = document.createElement("option");
                H.setAttribute("value", J);
                    ...
                    try {
                        if (localStorage) {
                            for (var K = 0; K < localStorage.length; K++) {
                                var J = localStorage.key(K);
                                if (J.substring(0, H.length) === H) {
                                    I.addOption(localStorage.getItem(J), J.substring(H.length))
                                }
                            }
                        }
                    } catch (L) {
            ...
            var q = function(H) {
                document.getElementById(H).style.backgroundColor = "lightcoral";
                window.setTimeout("document.getElementById('" + H + "').style.backgroundColor = 'white';", 1000)
            };
            ...
            var s = function() {
                var J = parseFloat(document.getElementById("currentLatitude").value);
                var I = parseFloat(document.getElementById("currentLongitude").value);
                var H = parseFloat(document.getElementById("currentAltitude").value);
                if (J != null && AR.VALIDATE.isInRange(J, -90, 90)) {} else {
                    q("currentLatitude");
                    return
                }
                if (I != null && AR.VALIDATE.isInRange(I, -180, 180)) {} else {
                    q("currentLongitude");
                    return
                }
                if (H != null && AR.VALIDATE.isNumeric(H) && !isNaN(H)) {} else {
                    q("currentAltitude");
                    return
                }
                return [J, I, H]
            ...
            m.onclick = function() {
                var H = s();
                if (H) {
                    AR.ADE.instance.applyLocationUpdate(H)
                }
            };
            C.appendChild(B);
            w.appendChild(C);
            ...
            o.onclick = function() {
                ...
                if (H) {
                    AR.ADE.instance.applyLocationUpdate(H);
                    var I = prompt("Enter the ID the saved location:", "");
                    if (I) {
                        r.addOption(AR.__toJSONString__(H), I, true);
                        I = i + I;
                        if (!localStorage) {
                            alert("Your Browser does not support local storage. Values will not be persisted.");
                            return
                        }
                        localStorage.setItem(I, AR.__toJSONString__(H))
                    }
                }
            };
            F.appendChild(o);
                    ...
                    if (f instanceof AR.Tracker) {
                        ...
                        if (f instanceof AR.HtmlDrawable) {
                            ...
                            if (f instanceof AR.Sound) {
                                ...
                                if (f instanceof AR.VideoDrawable) {
                                    AR.i.videoDrawableInterface.simulateLoad({
                                        objectId: f.__id
                                    })
                                } else {
    ...
    AR.ADE.Tree = function(b) {
        ...
        var k = function(s, o, r, p) {
            ...
            if (r.active == null || r.active != false) {
                q.href = n(s, r)
            }
            this.node = new YAHOO.widget.TextNode(q, p.node);
            this.update = function(t) {
    ...
    AR.i.htmlDrawableInterface = {
        ...
        setViewportHeight: function(a) {
            AR.i.getBridgeObject(a.objectId)._viewportHeight = a.viewportHeight;
            AR.ADE.instance.alertModification(a.objectId)
        },
        ...
        createHtmlDrawable: function(b) {
            ...
            AR.i.registerBridgeObject({
                ...
                _viewportHeight: b.viewportHeight,
                _width: b.width,
    ...
    AR.i.contextInterface = {
        ...
        setDistanceBasedScalingParameters: function(a) {
            AR.i.contextInterface._scalingFactor = a.dbs;
            AR.i.contextInterface._minScalingDistance = a.dCutOffMin;
            AR.i.contextInterface._maxScalingDistance = a.dCutOffMax;
            AR.ADE.instance.alertModification("context")
        },
    ...
    AR.i.videoDrawableInterface = {
        ...
        setOnPlaybackStartedTriggerActive: function(a) {
            AR.i.getBridgeObject(a.objectId)._onPlaybackStartedTriggerActive = a.onPlaybackStartedTriggerActive;
            AR.ADE.instance.alertModification(a.objectId)
        },
        setOnFinishedPlayingTriggerActive: function(a) {
            AR.i.getBridgeObject(a.objectId)._onFinishedPlayingTriggerActive = a.onFinishedPlayingTriggerActive;
